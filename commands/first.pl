...
if (length($arg_first < 1)){ $arg_first == $user; }
...
my $query = "SELECT timestamp, nick, chat FROM $Config{log} WHERE UPPER(nick) = UPPER(?) ORDER BY id ASC LIMIT 1";
my $handle = $dbh->prepare($query);
$handle->execute($arg_first);
my ($timestamp, $nick, $chat) = ('', '', '');
$handle->bind_columns(\$timestamp, \$nick, \$chat);
while ($handle->fetch()) {
  $message = "$nick was first heard from at $timestamp saying $chat";
}
$dbh->disconnect;
